    ...
    def get_event(self, event):
        ...
        if event.type == pg.MOUSEBUTTONDOWN:
            ...
            for pos, chara in self.pos2Chara.items():
                # 玩家1区域 (左侧)
                icon_rect = pg.Rect(c.P1_CHOOSE_BASE[0] + pos[0], c.P1_CHOOSE_BASE[1] + pos[1], 50, 50)
                if icon_rect.collidepoint(mouse_pos) and not self.p1_confirm:
                    self.p1_offset = pos
                    self.p1_confirm = True
                    break
                
                # 玩家2区域 (右侧)
                icon_rect2 = pg.Rect(c.P2_CHOOSE_BASE[0] + pos[0], c.P2_CHOOSE_BASE[1] + pos[1], 50, 50)
                if icon_rect2.collidepoint(mouse_pos) and not self.p2_confirm:
                    self.p2_offset = pos
                    self.p2_confirm = True
                    break
