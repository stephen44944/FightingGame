...
from ..components import special_bricks  # 特殊地图元素
from ..components import image_cache  # 图片缓存
import random
import threading
...
class Gaming(tools._State):
    ...
    def startup(self, current_time, persist):
        ...
        self.setup_MPsphere()
        self.setup_icons()
        self.setup_special_bricks()  # 设置特殊地图元素
    ...
    def preload_images(self):
        """预加载所有需要的图片到缓存"""
        # 预加载角色图片
        for char_name in c.CHARACTERS:
            if char_name != c.RANDOM:
                size = c.CHARACTER_SIZE.get(char_name, (50, 50))
                image_cache.ImageCache.preload_character(char_name, size)
        
        # 预加载砖块图片
        image_cache.ImageCache.preload_bricks(tools.kindOfBrick, c.BRICK_SIZE)
        
        # 预加载道具图片
        for prop_name, prop_info in tools.kindOfProps.items():
            image_cache.ImageCache.get_image(prop_info['name'], (c.BRICK_WIDTH, c.BRICK_HEIGHT))
    
    # ========== 特殊地图元素 ==========
    def setup_special_bricks(self):
        """设置特殊地图元素"""
        self.special_bricks_group = Group()
        # 示例：添加一些特殊元素到地图中
        # 可以根据需要调整位置和数量
        
        # 添加传送门（成对出现）
        portal1 = special_bricks.Portal(200, 500, portal_id=0)
        portal2 = special_bricks.Portal(600, 500, portal_id=1)
        portal1.linked_portal = portal2
        portal2.linked_portal = portal1
        self.special_bricks_group.add(portal1, portal2)
        
        # 添加弹跳板
        jump_pad1 = special_bricks.JumpPad(350, 600, jump_power=18)
        jump_pad2 = special_bricks.JumpPad(450, 600, jump_power=18)
        self.special_bricks_group.add(jump_pad1, jump_pad2)
        
        # 添加治疗区域
        heal_zone = special_bricks.HealingZone(100, 800, heal_amount=5, interval=300)
        self.special_bricks_group.add(heal_zone)
        
        # 添加加速带
        speed_boost = special_bricks.SpeedBoost(300, 800, boost_factor=2.5, duration=2000)
        self.special_bricks_group.add(speed_boost)
    ...
    def update_all_sprites(self, keys):
        self.update_props()
        self.update_brick()
        self.update_special_bricks()  # 更新特殊地图元素
        for character in self.characters_group.sprites():
    ...
    def update_special_bricks(self):
        """更新特殊地图元素"""
        for special_brick in self.special_bricks_group.sprites():
            special_brick.update(self.current_time)
    ...
    def adjust_sprite_positions(self):
        ...
        self.check_special_bricks_collisions()  # 检查特殊地图元素碰撞
        ...
        if character:
            if bullet.owner != character.player_num:
                if (character.vincible):
                    ...
                    actual_damage = character.apply_passive_on_damage_received(
                        self.characters_group.sprites()[bullet.owner], original_damage)
                    character.HP -= actual_damage
                    
                    # ========== 被动技能：造成伤害时触发（攻击者）=========
                    attacker = self.characters_group.sprites()[bullet.owner]
                    attacker.apply_passive_on_damage_dealt(character, actual_damage)
                    
                if character.HP <= 0:
                    self.reset_character(character)
                    # ========== 被动技能：击杀时触发 ==========
                    attacker = self.characters_group.sprites()[bullet.owner]
                    attacker.apply_passive_on_kill(character)
    ...
    # ========== 特殊地图元素碰撞检测 ==========
    def check_special_bricks_collisions(self):
        """检查角色与特殊地图元素的碰撞"""
        for character in self.characters_group.sprites():
            for special_brick in self.special_bricks_group.sprites():
                if pg.sprite.collide_rect(character, special_brick):
                    special_brick.ActOnCharacter(character, self.current_time)
    ...
    def blit_everything(self, surface):
        ...
        # ========== 绘制特殊地图元素 ==========
        for special_brick in self.special_bricks_group.sprites():
            if hasattr(special_brick, 'image'):
                self.map.blit(special_brick.image, special_brick.rect)
