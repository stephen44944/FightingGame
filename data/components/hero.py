...
from .passive_skills import get_passive_skill
...
class Hero(Sprite):
    ...
    def __init__(self):
        ...
        # ========== 被动技能系统 ==========
        self.passive_skill = None  # 角色被动技能
        self.invisible_time = 0  # 隐身时间
        self.slow_time = 0  # 减速时间
        self.portal_cooldown = 0  # 传送门冷却
        self.ice_sliding = False  # 冰面滑行状态
        
        # ========== 状态效果 ==========
        self.slow_factor = 1.0  # 移动速度系数
        self.damage_bonus = 1.0  # 伤害加成系数
    ...
    # ========== 被动技能相关方法 ==========
    def setup_passive_skill(self, character_name):
        """设置角色的被动技能"""
        self.passive_skill = get_passive_skill(character_name)
        if self.passive_skill:
            self.passive_skill.attach(self)
    
    def update_passive_skill(self):
        """更新被动技能效果"""
        if self.passive_skill:
            self.passive_skill.on_update()
    
    def apply_passive_on_damage_dealt(self, target, damage):
        """被动技能：造成伤害时"""
        if self.passive_skill:
            return self.passive_skill.on_damage_dealt(target, damage)
        return damage
    
    def apply_passive_on_damage_received(self, attacker, damage):
        """被动技能：受到伤害时"""
        if self.passive_skill:
            return self.passive_skill.on_damage_received(attacker, damage)
        return damage
    
    def apply_passive_on_kill(self, target):
        """被动技能：击杀敌人时"""
        if self.passive_skill:
            self.passive_skill.on_kill(target)
    
    # ========== 状态效果相关方法 ==========
    def apply_slow(self, duration):
        """应用减速效果"""
        self.slow_time = self.current_time + duration
        self.slow_factor = c.SLOW_EFFECT
    
    def apply_freeze(self, duration):
        """应用冰冻效果"""
        self.freeze_time = self.current_time
        self.state = c.FREEZING
    
    def update_status_effects(self):
        """更新状态效果"""
        # 减速效果结束
        if self.slow_time and self.current_time > self.slow_time:
            self.slow_factor = 1.0
            self.slow_time = 0
        
        # 冰面滑行状态重置
        if self.state != c.WALKING:
            self.ice_sliding = False
